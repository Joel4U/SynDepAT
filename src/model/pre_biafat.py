...
class PreBiafAT(nn.Module):
    def __init__(self, config):
        ...
        self.ner_shared_encoder = TransformerEncoder(d_model=input_dim, num_layers=config.shared_enc_nlayers, n_head=8,
        ...
        self.dep_shared_encoder = TransformerEncoder(d_model=input_dim, num_layers=config.shared_enc_nlayers, n_head=8,
                                              feedforward_dim=2 * input_dim, attn_type=config.shared_enc_type, output_dim=config.enc_dim, dropout=config.shared_enc_dropout)
    ...
    def forward(self, Task: str, subword_input_ids: torch.Tensor,  word_seq_lens: torch.Tensor,
        ...
        if Task == 'dep':
            ...
            # shared dep encoder
            shared_enc_out = self.dep_shared_encoder(word_rep, non_pad_mask)
            ...
            # shared ner encoder
            shared_enc_out = self.ner_shared_encoder(word_rep, non_pad_mask)
            concat_enc_out = self.fusion(private_enc_out, shared_enc_out)
            if is_train:
                concat_enc_out = timestep_dropout(concat_enc_out, self.concat_dropout)
            ...
            if is_train:
                ...
                return torch.log(torch.stack(losses).mean() + 1) + adv_loss
            else:
                batch_y_pred = []
